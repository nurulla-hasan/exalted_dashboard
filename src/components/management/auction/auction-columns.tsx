...
export const auctionColumns: ColumnDef<Auction>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm text-muted-foreground">{row.original.incrementValue}</span>
    ),
  },
  {
    ...
    header: "Financing",
    cell: ({ row }) => (
      <span className="text-sm text-muted-foreground">{row.original.financing}</span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm text-muted-foreground">{row.original.startingDateTime}</span>
    ),
  },
  {
    accessorKey: "status",
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant={variant} className="rounded-full px-3 py-1 text-xs font-normal">
          {status}
        </Badge>
      );
    },
